...
threats <- c(0,5,10,15,20,25,30,35,40,45,50,55,60)
...
row1 <- magick::image_append(c(plots_contributions[[1]], plots_contributions[[2]], plots_contributions[[3]], plots_contributions[[4]], plots_contributions[[5]]), stack = FALSE)
row2 <- magick::image_append(c(plots_punishments[[1]], plots_punishments[[2]], plots_punishments[[3]], plots_punishments[[4]], plots_punishments[[5]]), stack = FALSE)
...
for (i in 1:10) {
  posX <- x_pos + ((i - 1) %% 5) * x_increment # Modulus 5 since there are 5 images per row
  posY <- y_pos + ((i - 1) %/% 5) * y_increment # Integer division by 5 to switch between rows
...
replaceLineIfNeeded <- function(filename) {
  ...
  if (length(data) < 3002) {
    print(paste("File", filename, "contains fewer than 3002 lines. No changes made."))
    return(FALSE)
  }
  ...
  if (length(line3002) < 4) {
    ...
    print(paste("Line 3002 in file", filename, "has sufficient elements."))
  }
}
...
for (probability in probabilities) {
  for (threat in threats) {
    for (fileType in fileTypes) {
      for (repl in 1:3) {
        filename <- paste(fileType, '_test2PG_b3.0c1.0l0.5rho1.5i1e0mu0.01death0.1im1bP30t', threat, '.0p', probability, 'repl', repl, '.0.txt', sep = '')
